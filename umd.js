    ...
    global.Universal = function (name, defaults, callback, root) {
            ...
            for (var key in dependencies) {
                if (dependencies.hasOwnProperty(key)) {
                    if (overrides.hasOwnProperty(key)) {
                        mergedDependencies.push(overrides[key]);
                    } else {
                        mergedDependencies.push(dependencies[key]);
                    }
                }
            }
        ...
        var mergeDependencyDefinitionsForGlobals = function (overrides) {
            ...
            for (var key in dependencies) {
                if (dependencies.hasOwnProperty(key)) {
                    if (overrides.hasOwnProperty(key)) {
                        var overrideReference = overrides[key];
                        if (isString(overrideReference)) {
                            overrideReference = root[overrideReference];
                        }
                        mergedDependencies.push(overrideReference);
                    } else {
                        if (root.hasOwnProperty(dependencies[key])) {
                            mergedDependencies.push(root[dependencies[key]]);
                        } else {
        ...
        var mergeSettings = function (options) {
            ...
            for (var key in defaults) {
                ...
                // dependencies should not be considered a setting
                if (key === 'dependencies') {
                    continue;
                }
